...
st.markdown("""
    ...
    /* Estilo personalizado para el checkbox de confirmación: Letra Roja, Calibri, Tamaño Mayor */
...
def cargar_datos(ruta_archivo):
    try:
        df = pd.read_excel(ruta_archivo, dtype={'N°. de Cédula de Identidad': str})
        df['N°. de Cédula de Identidad'] = df['N°. de Cédula de Identidad'].astype(str).str.strip()
        return df, None
...
columnas_requeridas = ['Nombres', 'Apellidos', 'N°. de Cédula de Identidad']
...
if boton_consultar or st.session_state.get('verificado', False):
    ...
    if not cedula_limpia:
        ...
        resultado = df_egresados[df_egresados['N°. de Cédula de Identidad'] == cedula_limpia]
        ...
        if not resultado.empty:
            ...
            cedula_alumno = datos_alumno['N°. de Cédula de Identidad']
            ...
            st.write(f"**N° de Cédula de Identidad:** {cedula_alumno}")
            ...
            st.markdown("""
            1. **Confirmar la exactitud de datos** (verifique que sus nombres, apellidos y cédula sean correctos).
            ...
            # --- CASILLA CON FUENTE CALIBRI, TAMAÑO MAYOR Y COLOR ROJO ---
